...
def arm_and_takeoff(master, altitude=5):
    ...
    for _ in range(10):  # Check for up to 10 seconds
        ...
        if msg:
            if msg.get_type() == 'HEARTBEAT' and msg.base_mode & mavutil.mavlink.MAV_MODE_FLAG_SAFETY_ARMED:
                print('✅ Drone confirmed as armed.')
                break
            elif msg.get_type() == 'SYS_STATUS' and msg.onboard_control_sensors_enabled:
                print('✅ Drone sensors activated, ready for flight.')
                break
        else:
            print('⏳ Waiting for drone to arm...')
            time.sleep(1)
    else:
        print('❌ Drone arming timeout!')
        return
    ...
    time.sleep(10)  # Give some time for the drone to ascend
    print(f'✅ Drone should be at {altitude}m altitude.')
# Return to Launch
